...
int FCGX_VFPrintF(FCGX_Stream *stream, const char *format, va_list arg)
{
    ...
    int op, performedOp, sizeModifier, buffCount = 0, specifierLength;
...
static int ProcessManagementRecord(int type, FCGX_Stream *stream)
{
    ...
    if(type == FCGI_GET_VALUES) {
        ...
        FCGI_Header h = MakeHeader(FCGI_GET_VALUES_RESULT, FCGI_NULL_REQUEST_ID, len, paddedLen - len);
        memcpy(response,&h, sizeof(FCGI_Header));
        FreeParams(&paramsPtr);
    } else {
        paddedLen = len = sizeof(FCGI_UnknownTypeBody);
        FCGI_Header h = MakeHeader(FCGI_UNKNOWN_TYPE, FCGI_NULL_REQUEST_ID, len, 0);
        FCGI_UnknownTypeRecord * r = (FCGI_UnknownTypeRecord *) response;
        memcpy(&r->header, &h, sizeof(FCGI_Header));
        FCGI_UnknownTypeBody b = MakeUnknownTypeBody(type);
        memcpy(&r->body, &b, sizeof(FCGI_Header));
