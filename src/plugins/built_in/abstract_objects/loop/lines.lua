...
module("loop.lines",package.seeall)

-- DEPRECATED, use loop:get_src_lines instead
function loop:get_src_line()
   Message:warn ("loop:get_src_line is deprecated and will be removed. Use loop:get_src_lines instead (default return values are 0 instead of -1).\n");
   ...
   --print("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
   --print("Looking SRC LINE FOR LOOP "..self:get_id());
...
function loop:get_asm_addresses()
   local src_start = -1;
   local src_end = -1;
   local block,insn

   --print("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
   --print("Looking ASM ADDRESSES FOR LOOP "..self:get_id());
   for block in self:blocks() do
      for insn in block:instructions() do
